...
theorem sum1 (ms : List ℕ) (n : ℕ) : sum (snoc ms n) = n + sum ms := by sorry

theorem sum2 (ms ns : List ℕ) : sum (ms ++ ns) = sum ms + sum ns := by sorry

theorem sum3 (ns : List ℕ) : sum (reverse ns) = sum ns := by sorry
